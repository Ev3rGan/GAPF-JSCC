...
> **公开研究配套仓库。** 本仓库提供 GAPF-JSCC 的问题定义、方法谱系、证据边界与论文辅助材料；当前公开内容用于理解方法，不构成完整训练或端到端复现包。
...
| GAPF 端到端评测 | 所选条件下的系统级结果与定性比较已进入工作论文；仓库主张台账尚待同步 | 以最终论文和更新后的证据台账为准；当前仓库不新增逐组件归因或普适领先主张 |
...
当前论文仍处于审阅阶段。仓库已公开三份论文辅助材料：现行写作规范、成稿验收标准以及公式与符号审计表；工作论文的 TeX、PDF、图件和逐轮内部记录暂不作为公开仓库内容。详见 [paper/README.md](paper/README.md)。
...
仓库只提供原创分析、文献元数据和官方来源链接，默认不上传第三方 PDF。项目许可证尚未确定；在许可证和上游再分发边界确认前，本仓库内容不可被视为已授予代码再分发许可。

## 审阅与批准状态

作者已批准仓库公开及本轮文档更新。当前发布范围与仍保留的边界见 [REVIEW.md](REVIEW.md)。
